...
<head>
  ...
  <title>Test Link Manage | ECI</title>
...
?>
  ...
  <div class="content-wrapper">
    ...
    <div class="form-group">
      <button style="text-align:left" type="submit" href="add-new-students.php" class="btn btn-primary">Add New Link</button>               
    </div>
      <!-- Default box -->
      <div class="card">
      <?php
      ...
      if (isset($_GET['delete'])) {
        # code...
        $link_id = $_GET['delete'];
        $delete_query = "DELETE FROM `add_link` WHERE `link_id` = $link_id";
...
<button type="submit" name="search" class="btn btn-info float">Search</button>
          ...
          <table class="table table-striped projects">
              ...
              if(isset($_POST['search'])){
                ...
                $i=1;
$query_link_table = "SELECT * FROM `add_link` WHERE `add_link`.`class_number` = $class_number AND `add_link`.`board_name` LIKE '$board_name' AND `add_link`.`subject_id` = $subject_id ORDER BY `link_id` DESC";
$select_link = mysqli_query($connection,$query_link_table);
while ($get_link = mysqli_fetch_assoc($select_link)) {
  $link_id = $get_link['link_id'];
  $link_desc = $get_link['link_desc'];
  $link_url = $get_link['link_url'];
//   $filename = $study_material['filename'];
//   $study_material_id = $study_material['study_material_id'];

  $time_stamp = $get_link['time_stamp'];
...
//   $filename = $file['filename'];
                ...
                //   echo"</td>";
                  echo"<td>";
                  echo" <a href='test_link_manage.php?delete=$link_id' class='btn btn-danger'>Delete</a>";
                  echo"</td> ";
                  echo"</tr>";
