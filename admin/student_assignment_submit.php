        ...
        <div class="card-body p-0">
          ...
          <table class='table table-striped projects'>
            ...
            if (isset($_GET['assignment_id'])) {
              ...
              $i=1;
              $query_student_assignment = "SELECT * FROM `student_assignment` WHERE `student_assignment`.`assignment_id`= $assignment_id";
              $select_student_assignment = mysqli_query($connection,$query_student_assignment);
              while($student_assignment_row = mysqli_fetch_assoc($select_student_assignment)){
                $student_assignment_roll = $student_assignment_row['student_roll'];
                $filename = $student_assignment_row['filename'];
                $status = $student_assignment_row['status'];

                $query_student = "SELECT * FROM `student` WHERE `student`.`student_roll`= '$student_assignment_roll'";
                $select_student = mysqli_query($connection,$query_student);
                while($student_row = mysqli_fetch_assoc($select_student)){  
                  $student_name = $student_row['student_name'];
                  $student_roll = $student_row['student_roll'];
                ...
                <input type='hidden' id='student_enroll' name='student_enroll' value='$student_roll'>
                <td>
                $student_name
                </td>
                <td>
                <a href='../uploads/$filename' download='$student_name'>Download</a>
                </td>
                <td>
                <select class='form-control select2' name='assignment_status' style='width: 100%;'>
                                      <option value='0'>Pending</option>
                                      <option value='1'>Submitted</option>
                                      <option value='2'>Complete</option>
                                      <option value='3'>Rejected</option>
                                      <option value='4'>Late submit</option>
                                      <option value='5'>Late complete</option>
                                      <option value='6'>Late rejected</option>
                                      </select>
                </td>
                <td>";
                if ($status == 0) {
                ...
                echo"</td>
<td><button type='submit' name='update_status' class='btn btn-info float'>Update status</button></td>
              </tr>
              </form>";
            ...
            if (isset($_POST['update_status'])) {
              echo $student_enroll = $_POST['student_enroll'];
              echo $assignment_status = $_POST['assignment_status'];
              echo $assignment_id = $_GET['assignment_id'];
              $query = "UPDATE `student_assignment` SET `status` = '$assignment_status' WHERE `student_assignment`.`assignment_id` = $assignment_id AND `student_assignment`.`student_roll` = '$student_enroll'";
              $select_student_assignment = mysqli_query($connection,$query);
              if (!$select_student_assignment) {
                printf("Error message: %s\n", mysqli_error($connection));
              }
              header("Location: student_assignment_submit.php?assignment_id=$assignment_id");
