...
<head>
  ...
  <title>Student News Manage | ECI</title>
...
?>
  ...
  <div class="content-wrapper">
    ...
    <div class="form-group">
      <button style="text-align:left" type="submit" href="add-new-students.php" class="btn btn-primary">Add New News</button>               
    </div>
      <!-- Default box -->
      <div class="card">
      <?php
      ...
      if (isset($_GET['delete'])) {
        # code...
        $news_id = $_GET['delete'];
        $delete_query = "DELETE FROM `news_for_student` WHERE `news_id` = $news_id";
...
<button type="submit" name="search" class="btn btn-info float">Search</button>
          ...
          <table class="table table-striped projects">
              ...
              if(isset($_POST['search'])){
                ...
                $i=1;
$query_news_table = "SELECT * FROM `news_for_student` WHERE `news_for_student`.`class_number` = $class_number AND `news_for_student`.`subject_id` = $subject_id AND `news_for_student`.`board_name` LIKE '$board_name' ORDER BY `news_id` DESC";
$select_news = mysqli_query($connection,$query_news_table);
while ($news_row = mysqli_fetch_assoc($select_news)) {
  $news_id  = $news_row['news_id'];
  $student_roll  = $news_row['student_roll'];
  $assignment  = $news_row['assignment'];
  $publish_date  = $news_row['publish_date'];
  $time_stamp  = $news_row['time_stamp'];
...
//   $filename = $file['filename'];
                ...
                //   echo"</td>";
                  echo"<td>";
                  echo" <a href='student_news_manage.php?delete=$news_id' class='btn btn-danger'>Delete</a>";
                  echo"</td> ";
                  echo"</tr>";
